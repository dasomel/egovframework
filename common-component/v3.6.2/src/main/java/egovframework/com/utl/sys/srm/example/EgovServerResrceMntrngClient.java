	...
	public void connect() throws Exception {
		try {
          ...
          //KISA 보안약점 조치 (2018-10-29, 윤창원)
            LOGGER.debug("MBean info : " + mBeanInfo.getClassName());

            for (MBeanAttributeInfo attrInfo : attrInfos) {
            	if (attrInfo.getName().equals("CpuUsage")) {
            		LOGGER.debug("CPU : " + mbs.getAttribute(name, attrInfo.getName()).toString());
            	} else if (attrInfo.getName().equals("MemoryUsage")){ 
            		LOGGER.debug("Memory : " + mbs.getAttribute(name, attrInfo.getName()).toString());
            	} else {
            		LOGGER.debug(attrInfo.getName() + " = " + mbs.getAttribute(name, attrInfo.getName()));
            	}
            }
        
    	} catch (IOException ex) {//KISA 보안약점 조치 (2018-10-29, 윤창원)
    		LOGGER.error("["+ex.getClass()+"] server connection (IOException): " + ex.getMessage());
    		//throw new RuntimeException(ex);
    		throw new EgovBizException("[server resource monitoring] : connector.getMBeanServerConnection fail");
        //2017.02.07 	이정은 	시큐어코딩(ES)-오류 메시지를 통한 정보노출[CWE-211]
    	} catch (Exception ex) {
